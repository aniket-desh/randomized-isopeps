...
def run(args: argparse.Namespace) -> tuple[str, str]:
    ...
    csv_path = f"outputs/data/exp1-local-{stamp}.csv"
    fig_path = f"outputs/figures/exp1-local-{stamp}.pdf"
    write_csv(csv_path, rows)
    make_plot(rows, fig_path)
    return csv_path, fig_path


def mean_by_eta_mode(rows: list[dict[str, object]], key: str) -> dict[str, tuple[list[int], list[float]]]:
    grouped: dict[tuple[str, int], list[float]] = defaultdict(list)
    for row in rows:
        grouped[(str(row["mode"]), int(row["eta"]))].append(float(row[key]))
    out: dict[str, tuple[list[int], list[float]]] = {}
    for mode in MODES:
        etas = sorted({eta for m, eta in grouped if m == mode})
        values = [float(np.mean(grouped[(mode, eta)])) for eta in etas]
        out[mode] = (etas, values)
    return out


def make_plot(rows: list[dict[str, object]], fig_path: str) -> None:
    panels = [
        ("local two-svd timing", "total_time_s", "runtime (s)", "linear"),
        ("compression error", "rel_error", "relative reconstruction error", "log"),
        ("isometry check", "second_iso_defect", "second isometry defect", "log"),
    ]
    svg_panels: list[Panel] = []
    for title, key, ylabel, yscale in panels:
        series = mean_by_eta_mode(rows, key)
        svg_panels.append(
            Panel(
                title=title,
                xlabel="eta",
                ...
                series=[
                    Series(
                        ...
                        x=[float(v) for v in etas],
                        y=values,
                        color=PALETTE[mode],
                        marker=MARKERS[mode],
                    )
                    for mode, (etas, values) in series.items()
                ],
            )
        )
    write_line_panels(fig_path, svg_panels, width=1140, height=420)
